...
local function ParseLightTable(lt, interior, default_falloff)
    ...
    -- use manually defined lowcolor lights or try to use poweron lights if not defined to keep dlights on during lowpower mode
    if lt.lowpower then
        lt.low_color = lt.low_color or lt.color
        lt.low_pos = lt.low_pos or lt.pos
        lt.low_brightness = lt.low_brightness or lt.brightness
        lt.low_falloff = lt.low_falloff or lt.falloff
    end
    ...
    if lt.lowpower then
        lt.low_pos_global = interior:LocalToWorld(lt.low_pos)
        lt.low_color_vec = lt.low_color:ToVector() * lt.low_brightness
    ...
    if lt.lowpower then
        lt.low_render_table = {
            type = MATERIAL_LIGHT_POINT,
            color = lt.low_color_vec,
            pos = lt.low_pos_global,
            quadraticFalloff = lt.low_falloff,
        }
        -- lt.off_warn_render_table = {
        --     type = MATERIAL_LIGHT_POINT,
        --     color = lt.off_warn_color_vec,
...
if CLIENT then
    ...
    function ENT:DrawLight(id,light)
        ...
        local lowpower = self:GetData("lowpowermode", false)
        ...
        if lowpower then
            dlight.Pos = light.low_pos_global
            dlight.r = light.low_color.r
            dlight.g = light.low_color.g
            dlight.b = light.low_color.b
            dlight.Brightness = light.low_brightness
...
if CLIENT then
    ...
    local function SelectLampTable(self, lmp)
        ...
        if GetConVar("hugoextension_tardis2_NewPowerSystem"):GetBool() == true then -- allow players to disable all of this garbage below if it looks too bad or breaks extensions

            if not power and (not lowpower) then  -- stupidly bold move here - just ignore if a given lamp has nowpower enabled and instead depend on lowpower/nopower to decide
            ...
            if not power and lmp.nopower ~= true then  -- default lamp behavior
                return nil
            end
