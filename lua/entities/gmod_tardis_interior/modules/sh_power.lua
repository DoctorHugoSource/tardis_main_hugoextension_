...
if SERVER then
    ...
    ENT:AddHook("ShouldNotDrawScreen", "power", function(self)
        if not self:GetPower() and (not self:GetData("lowpowermode",true)) then
            return true
        end
    end)

    ENT:AddHook("ShouldDrawBlackScreen", "power", function(self)
        if not self:GetPower() and (not self:GetData("lowpowermode",true)) then  -- why does self:GetPowerMode() not work here??
            return true
        end
    end)
