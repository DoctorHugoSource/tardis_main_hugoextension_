...
ENT:AddHook("CanUseTardisControl", "power", function(self, control, ply)
    if self:GetData("lowpowermode", true) then return true end  -- attempt to let the player operate some, but not all, controls in low power mode
    if (not self:GetPower() and not control.power_independent) then  -- doesn't work flawlessly, will likely revisit at some point
