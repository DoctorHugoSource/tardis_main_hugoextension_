...
if SERVER then
    ...
    ENT:OnMessage("demat", function(self, data, ply)
        ...
        if TARDIS:GetSetting("teleport-sound") and TARDIS:GetSetting("sound") then
            ...
            local mathopint = int.mathop
            local mathopext_takeoff = ext.mathop_demat
            local mathopext_land = ext.mathop_mat
            ...
            if LocalPlayer():GetTardisData("exterior")==self then
                if self:GetFastRemat() and self:GetMatHop() then
                    if shouldPlayInterior then
                        self.interior:EmitSound(mathopint)
                    end
                    if shouldPlayExterior then
                        self:EmitSound(mathopext_takeoff)
                    end
            ...
            elseif shouldPlayExterior then
                if self:GetFastRemat() and self:GetMatHop() then
                    sound.Play(mathopext_takeoff,self:GetPos())
                elseif self:GetFastRemat() then
                ...
                if pos and self:GetFastRemat() and self:GetMatHop() then
                    if not IsValid(self) then return end
                        sound.Play(mathopext_land, pos)
                elseif pos and self:GetFastRemat() then
                    if not IsValid(self) then return end
...
function ENT:SetStepDelay()
    ...
    if veryfast and demat then                                      -- THIS LITTLE PART is very important because WITHOUT IT like three different syncing issues appear during the mathop sequence
        sequence_delays = teleport_md.DematVeryFastSequenceDelays   -- for one if you mathop right after a regular fast remat it will use the wrong sequence delay at the start and fuck up the timing
    end                                                             -- and if you mathop after a regular vortex flight it will get stuck for like 3 seconds before remat for some reason

    if veryfast and mat then
        sequence_delays = teleport_md.MatVeryFastSequenceDelays
    end
...
function ENT:GetTargetAlpha()
    ...
    local step = self:GetData("step",1)
    local fast = self:GetFastRemat()
    local fastdemat = fast and demat
    local fastmat = fast and mat
    local veryfast = self:GetData("mathop")
    local veryfastmat = mat and veryfast
    local veryfastdemat = demat and veryfast

    if demat and hads then
        return self.metadata.Exterior.Teleport.HadsDematSequence[step]
    elseif veryfastdemat and (not mat) then
        return self.metadata.Exterior.Teleport.DematSequenceVeryFast[step]
    elseif veryfastmat and (not demat) then
        return self.metadata.Exterior.Teleport.MatSequenceVeryFast[step]
    elseif fastdemat and (not mat) then
        return self.metadata.Exterior.Teleport.DematSequenceFast[step]
    elseif fastmat and (not demat) then
        return self.metadata.Exterior.Teleport.MatSequenceFast[step]
...
ENT:AddHook("Think","teleport",function(self,delta)
    ...
    local teleport_md = self.metadata.Exterior.Teleport
    local fast = self:GetFastRemat()
    local veryfast = self:GetMatHop() and self:GetFastRemat()

    local demat_steps --[[= hads and #teleport_md.HadsDematSequence or #teleport_md.DematSequence]]

    if hads then
        demat_steps = #teleport_md.HadsDematSequence
    elseif veryfast then
        demat_steps = #teleport_md.DematSequenceVeryFast
    elseif fast and (not veryfast) then
        demat_steps = #teleport_md.DematSequenceFast
    else
        demat_steps = #teleport_md.DematSequence
    end
    ...
    if alpha==target then
        ...
        elseif mat then
            if step >= #teleport_md.MatSequence or (fast and step >= #teleport_md.MatSequenceFast) or (veryfast and step >= #teleport_md.MatSequenceVeryFast) then
                self:StopMat()              -- or fast and step >= #teleport_md.MatSequenceFast -- is this needed??
                return                                                           -- yes this is needed or else mat wont finish and get stuck at the thud
            else
                self:SetData("step",step+1)
                self:SetStepDelay()
            end
    ...
    if veryfast then
    sequencespeed = teleport_md.SequenceSpeedVeryFast
    end
    if self:GetData("hads-demat") then
...
function ENT:GetSequenceProgress()
    ...
    if fastdemat and (not mat) then                         -- is this needed??
        sequence = tp_metadata.DematSequenceFast
    end

    if fastmat and (not demat) then                         -- is this needed??
        sequence = tp_metadata.MatSequenceFast
    end

    if veryfast and demat then                         -- is this needed??
        sequence = tp_metadata.DematSequenceVeryFast
    end

    if veryfast and mat then                         -- is this needed??
        sequence = tp_metadata.MatSequenceVeryFast
    end
