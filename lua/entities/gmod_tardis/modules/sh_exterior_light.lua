...
if SERVER then
    ...
    ENT:AddHook("Draw", "light", function(self)
        ...
        if shouldon and (not shouldoff) then
            ...
            if self.lightpixvis and (not wp.drawing) and (halo.RenderedEntity()~=self) then
                ...
                -- alpha = alpha * math.sqrt(self:GetData("alpha",255) / 255)

                if self:GetData("demat",false) or self:GetData("mat",false) then
                alpha = math.Clamp( (self:GetData("alpha",255) / 1.8), 0, 200)
                end
    ...
    ENT:AddHook("Think", "light", function(self)
        ...
        -- mult = mult * math.sqrt(self:GetData("alpha",255) / 255)

        if self:GetData("demat",false) or self:GetData("mat",false) then
        mult = (self:GetData("alpha",255) / 255)
        end
