if SERVER then
    ...
    ENT:AddHook("ArtronDepleted", "teleport_and_poweroff", function(self)
        ...
        if self:GetPower() or self:GetLowPowerMode() then
            self:InterruptFlight()
            self:SetData("lowpowermodeenable", false, true)
            self:SetArtron(1)
            self:SetPower(true)             -- very hacky but i can't think of any other way around this right now since the game considers low power to be power off already
            self:SetPower(false)
