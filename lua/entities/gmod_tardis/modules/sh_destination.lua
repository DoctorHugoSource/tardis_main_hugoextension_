...
TARDIS:AddKeyBind("destination-90degrees",{
    name="Snap90Degrees",
    section="Destination",
    func=function(self,down,ply)
        if down and ply:GetTardisData("destination") then
            local prop = self:GetData("destinationprop")
            if IsValid(prop) then
                prop:SetAngles(prop:GetAngles() + Angle(0,-90,0))
            end
        end
    end,
    key=KEY_R,
...
if SERVER then
    ...
    ply:SetTardisData("vmlocoldisable", true, true)

    timer.Simple(0.05, function() -- necessary for the VM light override to update for some reason
...
-- ENT:AddHook("PreDrawPart","ghostbox",function(self)
--     local prop = self:GetData("destinationprop")
--     local ply = LocalPlayer()
--     if ply:GetTardisData("destination") then
--     if not (prop or prop:GetChildren()[1]) then return end
--     render.OverrideColorWriteEnable( true, false )
--     prop:DrawModel()
--     prop:GetChildren()[1]:DrawModel()
--     render.OverrideColorWriteEnable( false, false )
--     end
-- end)
